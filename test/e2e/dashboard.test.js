...
test('reception completes patient, booking, recall and status flows against real API', async ({ page }) => {
  ...
  try {
    ...
    await page.getByRole('button', { name: 'Appointments' }).click(); await page.getByRole('button', { name: '+ Book appointment' }).click(); await page.getByLabel('Patient').selectOption({ label: 'Avery Synthetic' }); await page.getByLabel('Optometrist').selectOption({ label: 'Demo Optometrist' });
    ...
    await page.getByLabel('Change status for Avery Synthetic').selectOption('checked_in'); await expect(page.getByText('checked in', { exact: true })).toBeVisible();
    await page.getByRole('button', { name: 'Recall queue' }).click(); await page.getByRole('button', { name: '+ Add recall' }).click(); await page.getByLabel('Patient').selectOption({ label: 'Avery Synthetic' }); await page.getByLabel('Due date').fill('2030-03-01'); await page.getByRole('button', { name: 'Save' }).click(); await expect(page.getByText('Due 2030-03-01')).toBeVisible();
