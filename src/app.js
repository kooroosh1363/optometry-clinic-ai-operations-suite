...
// No patient/business routes are exposed before authentication is implemented.
export function createApp({ ready }) {
  return createServer(async (req, res) => {
    ...
    if (req.url === '/health/live') return send(200, { status: 'alive', phase: 'foundation' });
    try {
      if (await ready()) return send(200, { status: 'ready', schema: '001' });
