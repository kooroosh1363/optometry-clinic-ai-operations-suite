...
export async function migrate(pool, sql) {
  ...
  try {
    ...
    const previous = await client.query('SELECT checksum FROM schema_migrations WHERE version = $1', ['001']);
    if (previous.rowCount && previous.rows[0].checksum !== checksum) throw new Error('Migration checksum mismatch');
    if (!previous.rowCount) {
      await client.query(sql);
      await client.query('INSERT INTO schema_migrations(version, checksum) VALUES ($1, $2)', ['001', checksum]);
    }
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  try { await migrate(pool, await migrationSql()); console.log('schema_001_ready'); }
