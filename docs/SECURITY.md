...
Not implemented: password login, SSO/MFA, persistent browser session/CSRF design, RLS, rate limiting, read-access audit, credential-management audit, immutable audit storage, production TLS/key management, backup/restore verification, consent history, retention/deletion, external delivery and monitoring. No CORS policy allows cross-origin browser clients. Keep HTTP access on loopback. A local schema-owner API role is not least-privilege production configuration. CI credentials are disposable test values.
